    ...
    std::function<void(const GenerationResponse&)> callback)
{
    ...
    // Prefill (batch)
    Timer prefill_timer;
    float* logits = model.prefill(prompt_tokens.data(), (int)prompt_tokens.size());
    if (!logits) {
    ...
    bool enable_thinking)
{
    ...
    result.n_tokens = (int)prompt_tokens.size();

    // Prefill
    Timer prefill_timer;
    float* logits = model.prefill(prompt_tokens.data(), result.n_tokens);
    double prefill_ms = prefill_timer.elapsed_ms();

    if (!logits) {
        fprintf(stderr, "prefill_only: prefill failed\n");
        return result;
    }

    result.tps = result.n_tokens / (prefill_ms / 1000.0);
    return result;
}
