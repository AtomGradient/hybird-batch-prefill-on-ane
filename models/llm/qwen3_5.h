...
public:
    ...
    // Batch prefill: process n_tokens at once, return logits for the last token.
    // Default implementation falls back to token-by-token forward().
    virtual float* prefill(const int* token_ids, int n_tokens) {
        float* logits = nullptr;
        for (int i = 0; i < n_tokens; i++) {
            logits = forward(token_ids[i], i);
            if (!logits) return nullptr;
        }
        return logits;
    }

    // Save model state (caches + logits) to binary file for hybrid pipeline.
    // n_prompt_tokens: number of tokens that were prefilled.
    // Returns true on success.
    virtual bool save_state(const char* path, int n_prompt_tokens) { (void)path; (void)n_prompt_tokens; return false; }
};
...
public:
    ...
    float* prefill(const int* token_ids, int n_tokens) override;
    void reset() override;
    int vocab_size() const override { return vocab_size_; }
    bool save_state(const char* path, int n_prompt_tokens) override;
...
private:
    ...
    // Weight name prefix (auto-detected: "model.language_model." or "language_model.model.")
    std::string wp_;
    // Whether norm weights use (1+w) offset format (true) or direct format (false)
    bool norm_offset_format_ = true;
    ...
    // Batch prefill helpers
    bool prefill_deltanet_core(int L, int B, int pos_start);
    bool prefill_full_attn_core(int L, int B, int pos_start);

    // Prefill buffers (batch up to ANE_SPATIAL tokens)
    static constexpr int PREFILL_BATCH = ANE_SPATIAL; // 32
    float* pf_x_ = nullptr;        // B * hidden_size
    float* pf_xnorm_ = nullptr;    // B * hidden_size
    float* pf_proj_ = nullptr;     // B * max_proj_out
    float* pf_attn_out_ = nullptr; // B * max_attn_dim
    float* pf_tmp_ = nullptr;      // B * hidden_size
    bool pf_allocated_ = false;
    void alloc_prefill_buffers();
    void free_prefill_buffers();
};
