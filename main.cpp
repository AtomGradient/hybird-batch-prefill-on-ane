...
static void print_usage(const char* prog) {
    ...
    fprintf(stderr, "  --prefill-only    Run prefill only (no decode), for hybrid pipeline\n");
    fprintf(stderr, "  --state-file <path> Save model state to file after prefill\n");
...
static int cmd_generate(LLMModel& model, Tokenizer& tokenizer, const Args& args) {
    ...
    // Prefill-only mode for hybrid pipeline
    if (args.prefill_only) {
        auto result = prefill_only(model, tokenizer, std::string(args.prompt), args.enable_thinking);
        fprintf(stderr, "Prefill: %d tokens, %.3f tokens-per-sec\n", result.n_tokens, result.tps);

        if (args.state_file) {
            Timer save_timer;
            if (!model.save_state(args.state_file, result.n_tokens)) {
                fprintf(stderr, "Error: failed to save state to %s\n", args.state_file);
                return 1;
            }
            fprintf(stderr, "State saved to %s (%.1f ms)\n", args.state_file, save_timer.elapsed_ms());
        }
        return 0;
    }
