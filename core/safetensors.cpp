...
float* SafeTensors::load_to_f32(const char* name, int64_t expected_numel) const {
    const SFTensor* t = find(name);
    if (!t) {
        fprintf(stderr, "Weight not found: %s\n", name);
        return nullptr;
    ...
    if (t->dtype == SFDtype::F32) {
        return load_f32_direct(name, expected_numel);
    }
    return load_bf16_to_f32(name, expected_numel);
}
